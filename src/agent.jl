...
Thrown from `on_llm_result` to pause the agent loop before any tool executes.
...
```julia
hooks = AgentHooks(
    on_llm_result = (agent, iter, msg) -> begin
...
"""
    RetryConfig(; max_retries, initial_delay, max_delay, multiplier, jitter, retry_on_status)
...
"""
    AgentHooks(; on_llm_call, on_llm_result, should_interrupt, on_tool_call, on_tool_result, on_complete)
...
|---|---|---|
| `on_llm_call` | `(agent, iteration)` | Before each LLM request |
| `on_llm_result` | `(agent, iteration, response)` | After each LLM response — before any tool executes |
...
Base.@kwdef struct AgentHooks
    on_llm_call      ::Union{Function, Nothing} = nothing  # (agent, iteration)
    on_llm_result    ::Union{Function, Nothing} = nothing  # (agent, iteration, response)
...
- `instructions::String`: The system prompt — what the agent does and how it behaves.
...
function run!(agent::Agent, input::String;
    ...
    turn = TurnEvent(agent.name, input)
    ...
    # Seed the conversation: system prompt (+ skill metadata) + session history + user message
    system_prompt = agent.instructions * _skills_prompt(all_skills)
    ...
    for iteration in 1:agent.max_iterations
        ...
        if isnothing(agent.output_type) || !isempty(all_tools)
            _fire(hooks.on_llm_call, agent, iteration)
            ...
            _fire(hooks.on_llm_result, agent, iteration, last_msg)
            ...
            if last_msg isa PT.AIToolRequest && !isempty(last_msg.tool_calls)
                ...
                for tool_msg in last_msg.tool_calls
                    ...
                    # Tool result is pushed to conversation first so session history
                    # is complete and subsequent turns have full context.
