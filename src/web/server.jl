...
function _handle_chat(req::HTTP.Request)
    ...
    merged_hooks = AgentHooks(
        on_llm_call      = base_agent.hooks.on_llm_call,
        on_llm_result    = base_agent.hooks.on_llm_result,
    ...
    run.task = Threads.@spawn begin
        try
            ...
            if e isa ApprovalTimeout
                run.status = :error
                _push_event(run, "error",
                    Dict("message" => "Approval timed out after $(e.timeout)s"))
            else
                run.status = :error
                _push_event(run, "error",
                    Dict("message" => sprint(showerror, e)))
            end
        finally
