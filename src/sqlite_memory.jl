...
function add_memory!(service::SQLiteMemoryService, content::String;
    ...
    DBInterface.execute(service.db, """
    ...
    """, (entry.id, entry.content, entry.user_id, entry.app_name,
          metadata_json, entry.source_session_id, entry.created_at))

    entry
end

function search_memory(service::SQLiteMemoryService, query::String;
                       user_id  ::String = "default",
                       app_name ::String = "NimbleAgents",
                       top_k    ::Int    = 5)
    result = DBInterface.execute(service.db,
    ...
    for row in rows
        ...
        entry = MemoryEntry(
            row.id, row.content, row.user_id, row.app_name,
            meta, ssid, row.created_at,
        )
...
function list_memories(service::SQLiteMemoryService;
