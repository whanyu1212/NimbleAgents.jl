...
const grep_tool = NimbleTool(
    ...
    callable = (args::Dict{Symbol,<:Any}) -> begin
        ...
        length(matches) > 200 && push!(matches, "... ($(length(matches) - 200) more matches)")
    ...
    elseif isfile(path)
        try
            for (i, line) in enumerate(eachline(path))
                !isnothing(match(rx, line)) &&
                    push!(matches, "$(path):$(i): $(line)")
            end
        catch
...
const find_files_tool = NimbleTool(
    ...
    callable = (args::Dict{Symbol,<:Any}) -> begin
        ...
        rx = try Regex(pattern) catch; Regex(replace(pattern, r"[.*+?^${}()|[\]\\]" => s"\\\0")) end
        results = String[]
