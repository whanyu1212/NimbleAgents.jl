...
function make_hooks(label::String, color::String)
    AgentHooks(;
        ...
        on_tool_call=(ag, name, args) -> tprintln(
            "  {$(color)}[$(label)]{/$(color)} → {bold}$(name){/bold}: $(get(args, :query, get(args, :task, "…")))",
        ),
        on_complete=(ag, result) -> tprintln(
            "  {$(color)}[$(label)]{/$(color)} {green}✓{/green} ($(length(string(result))) chars)",
        ),
    )
end
...
print(
    Panel(
        ...
        title="NimbleAgents — Research Pipeline",
...
section(
    "Step 1 · spawn_subagents — QueryPlanner refines each sub-topic into a targeted search query",
)
...
with(pbar) do
    tasks = [
        Threads.@spawn begin
            result = run!(
                topic_researcher, planned_queries[i]; session=session, verbose=false
            )
            update!(job)
            result
        end for i in eachindex(planned_queries)
    ]
...
edit_out = spawn_subagents(
    [(editor, "Polish this draft:\n\n$draft")]; session=session, verbose=false
)
