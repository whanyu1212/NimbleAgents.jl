...
@testset "_render_command — placeholder substitution" begin
    t = CLITool(
        ...
        command     = ["cp", "{src}", "{dst}"],
        args        = [
            "src" => CLIArg(String, "Source"),
            "dst" => CLIArg(String, "Destination"),
        ],
    )
...
@testset "CLITool — return_direct / return_artifact flags" begin
    t1 = CLITool(name="a", description=".", command=["echo"], args=Pair{String,CLIArg}[],
                 return_direct=true)
    t2 = CLITool(name="b", description=".", command=["echo"], args=Pair{String,CLIArg}[],
