...
@testset "ExternalAgentTool — _render_command_ext" begin
    t = ExternalAgentTool(
        ...
        args        = [
            "task"  => CLIArg(String, "Task"),
            "model" => CLIArg(String, "Model"),
        ],
    )
    cmd = NimbleAgents._render_command_ext(t, Dict{Symbol,Any}(:task => "hello world", :model => "fast"))
...
@testset "ExternalAgentTool — execution via _run_external_agent" begin
    ...
    result = NimbleAgents._run_external_agent(t, Dict{Symbol,Any}(:task => "hello from agent"))
...
@testset "ExternalAgentTool — return_direct / return_artifact flags" begin
    t1 = ExternalAgentTool(name="a", description=".", command=["echo"],
                           args=Pair{String,CLIArg}[], return_direct=true)
    t2 = ExternalAgentTool(name="b", description=".", command=["echo"],
                           args=Pair{String,CLIArg}[], return_artifact=true)
...
@testset "ExternalAgentTool — dispatch_tool integration" begin
    ...
    result = dispatch_tool(tool_map, "echo_dispatch_ext", Dict{Symbol,Any}(:task => "dispatch test"))
