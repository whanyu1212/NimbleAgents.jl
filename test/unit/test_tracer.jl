...
function _make_tool_event(;
    name="add", args=Dict{Symbol,Any}(:x=>1, :y=>2), result=3, error=nothing
)
...
@testset "Trace — single turn" begin
    te = _make_tool_event()
    turn = _make_turn(
        input_tokens=100, output_tokens=40, cost=0.005, elapsed=2.5, tools=[te], llm_calls=2
    )
    trace = Trace([turn])
...
@testset "Trace — multiple turns, multiple agents" begin
    ...
    t2 = _make_turn(
        ...
        cost=0.002,
        elapsed=2.0,
        tools=[_make_tool_event(), _make_tool_event()],
        llm_calls=2,
    )
    trace = Trace([t1, t2])
